...
export default function GroceryApproval() {
  ...
  // Fetch pending grocery approval requests
  const fetchGroceryRequests = async () => {
    try {
      setLoading(true)
      const response = await adminAPI.getPendingGroceryApprovals()
      const data = response?.data?.data?.requests || response?.data?.requests || []
      setGroceryRequests(data)
    } catch (error) {
      console.error('Error fetching grocery approval requests:', error)
      toast.error('Failed to load grocery approval requests')
      setGroceryRequests([])
    } finally {
      setLoading(false)
    }
  ...
  }, [groceryRequests, searchQuery])

  const totalRequests = filteredRequests.length

  // Handle approve grocery item
  const handleApprove = async (request) => {
    try {
      setProcessing(true)
      await adminAPI.approveGroceryItem(request._id || request.id)
      toast.success('Grocery item approved successfully')
      await fetchGroceryRequests()
      setShowDetailModal(false)
      setSelectedRequest(null)
    } catch (error) {
      console.error('Error approving grocery item:', error)
      toast.error(error?.response?.data?.message || 'Failed to approve grocery item')
    } finally {
      setProcessing(false)
    }
  }

  // Handle reject grocery item
  const handleReject = async () => {
    ...
    try {
      setProcessing(true)
      await adminAPI.rejectGroceryItem(selectedRequest._id || selectedRequest.id, rejectReason)
      toast.success('Grocery item rejected')
      await fetchGroceryRequests()
      ...
      console.error('Error rejecting grocery item:', error)
      toast.error(error?.response?.data?.message || 'Failed to reject grocery item')
    } finally {
      setProcessing(false)
    }
  }

  // View grocery item details
  ...
  return (
    <div className="p-6 space-y-4">
      ...
      {/* Grocery Approval List Section */}
      <Card className="border border-gray-200 shadow-sm">
        <div className="p-4">
          {/* Section Header */}
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
            <div className="flex items-center gap-2">
              <h2 className="text-base font-semibold text-gray-900">Pending Grocery Approvals</h2>
          ...
          <div className="mb-4">
            <div className="relative flex-1">
              ...
              <input
                type="text"
                placeholder="Ex: search by product name, store name or ID"
                value={searchQuery}
                    ...
                    {filteredRequests.length === 0 ? (
                      <tr>
                        <td colSpan="8" className="px-3 py-8 text-center text-sm text-gray-500">
                          {loading ? "Loading..." : "No pending grocery approval requests found."}
                        </td>
                      </tr>
                    ) : (
        ...
        <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto p-0 bg-white">
          <DialogHeader className="p-6 pb-4 border-b border-gray-200">
            ...
            <DialogDescription className="text-sm text-gray-500 mt-1">
              Review the product details before approval.
            </DialogDescription>
          </DialogHeader>
          {selectedRequest && (
            <div className="p-6 space-y-4">
